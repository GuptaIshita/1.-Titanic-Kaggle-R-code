...
library(ggplot2)
library(dplyr)
setwd("C:\\Users\\risin\\OneDrive\\Desktop\\jigsaw academy\\ZC. Public Projects_Kaggle_Github\\1. Titanic-Kaggle-R-code\\1.-Titanic-Kaggle-R-code\\data sets")
train=read.csv("train.csv")
names(train)
test=read.csv("test.csv")
names(test)
#train1 = select(train,-Survived)
#titanic<-rbind(train1,test)
titanic<-bind_rows(train,test)
head(titanic)
#################################univariate analysis###########################################
##############################################################################################
str(titanic)
# Numerical - Continuouos + Integer, Categorical - Categorical

# Continuous - Fare, Age  
# Categorical - Name, Sex, Ticket, Cabin, Embarked
# Integer -  PassengerId, PClass, Sibsp, Parch, Survived
...
# Age - 263 NA's
# SibSp - Mostly 0's, & Maximum value - 8
# Parch - Mostly 0's, & Maximum value- 9
# Fare amount varies from 8 t0 512. 1 NA
# Mostly to embark on Southampton, then Cherbourg and few on Queenstown (S->C->Q)
# Embarked - 2 NA's
# Survived - 418 NA's (test data - have to predict)
##############################################################################################
#####Treatment of missing values##########################################
# Age : positively skewed. imputed with median i.e. 28
# Fare : imputed with median as positively skewed
...
median(titanic$Fare,na.rm=TRUE) #14.45
titanic$Fare[is.na(titanic$Fare)]<-14.45
##########For Numerical variables : Histogram + Density Plot + summary stats#########################
# 1) Age
hist(titanic$Age)
...
# 2) Fare
hist(titanic$Fare)
d1<-density(titanic$Fare)
plot(d1)
summary(titanic$Fare)
##############################################################################################
########For Categorical variables : Barplot + Dataframe of absolute & relative frequency #########################
# 1) Sex
...
#5) Parch
ggplot(titanic,aes(x=Parch))+geom_bar()
...
####################################(1)boxplot###################################################
ggplot(titanic,aes(y=Age,x=Survived,fill=as.factor(Survived)))+geom_boxplot()
ggplot(titanic,aes(x=Age))+geom_histogram(aes(fill=as.factor(Survived)),position="dodge")

ggplot(titanic,aes(y=Fare,x=Survived,fill=as.factor(Survived)))+geom_boxplot()
ggplot(titanic,aes(x=Fare))+geom_histogram(aes(fill=as.factor(Survived)),position="dodge")
